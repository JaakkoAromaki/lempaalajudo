...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center gap-10 px-6 py-10">
      <h1 className={`${fredoka.className} text-4xl text-center text-black`}>Ryhmät</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-5xl">
        {groups.map((group, index) => (
          <div key={group.id || index} className="bg-gray-100 rounded-2xl p-6 flex flex-col items-center shadow-md">
            <h2 className={`${fredoka.className} text-2xl text-black mb-3 text-center`}>{group.name}</h2>
            <p className={`${openSans.className} text-base text-gray-700 mb-4 text-center`}>
              {group.description}
            </p>
            {group.videoEmbedUrl && (
              <div className="w-full aspect-video rounded-lg shadow">
                <iframe
                  ...
                  title={`YouTube video ryhmälle ${group.name}`}
                  frameBorder="0"
