...
const Login: React.FC = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <form onSubmit={handleLogin} className="bg-white p-6 rounded-lg shadow-lg w-80 md:w-96">
        <h1 className={`${fredoka.className} text-2xl md:text-3xl mb-6 text-center`}>
          Kirjaudu sisään
        </h1>

        {error && <p className="text-red-500 mb-3">{error}</p>}

        <label className="block mb-2 font-semibold">Sähköposti/Käyttäjänimi</label>
        <input
          type="email"
          value={email}
